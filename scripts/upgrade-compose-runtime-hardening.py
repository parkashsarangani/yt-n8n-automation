...
def upgrade(path:Path)->None:
    _upgrade(path);patch_retrieval_observability(path);compose_path=path.with_name('compose.js')
...
def main():
    ...
    path=Path(sys.argv[1]);upgrade(path);print(f'runtime-hardened + observable b-roll resolver written to {path}')
