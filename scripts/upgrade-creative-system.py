...
def patch_writer_editor(w: dict) -> None:
    ...
    if "ENGAGEMENT OPTIONAL - CREATIVE_SYSTEM" not in body:
        ...
        rules = (
            ...
            "22. STRUCTURAL FIELD INTEGRITY - CREATIVE_SYSTEM (this is the most common reason a Short gets discarded, so check it explicitly, not just as an afterthought): caption_style must be EXACTLY one of upbeat, serious, funny, neutral - no other word. trigger must be EXACTLY one of curiosity_gap, disbelief, fear_stakes, scale_shock, taboo_secret - no other word. hook must be 5-200 characters. hook_candidates, if present, must stay an array of 3-6 strings - never fewer, never more. If the draft is missing any of these fields, or has a value that is not exactly one of the allowed words, fix it to the closest correct value - do not leave it missing, blank, or slightly off. Do this check LAST, after every other rewrite, so a later edit cannot silently break it.\\n\\n" + anchor
        )
