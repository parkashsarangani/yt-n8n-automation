...
if(Array.isArray(parsed.scenes)&&parsed.scenes.length){
  ...
  if(!_pbHasValid){
    let _pbDraft;
    try{_pbDraft=$('Parse Draft JSON').item.json.draft&&$('Parse Draft JSON').item.json.draft.payoff;}catch(e){_pbDraft=undefined;}
