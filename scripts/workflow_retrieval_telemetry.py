...
annotations, and sampled-frame verification fields. Legacy keys remain so
existing analytics and later transforms continue to work.
"""
...
const retrieval={scene_index:sceneIndex,visual_mode:r.visual_mode||scene.visual_mode||null,must_show:scene.must_show||scene.visual_claim||null,requested_queries:Array.isArray(scene.search_queries)?scene.search_queries.slice(0,6):[],source_priority:Array.isArray(scene.source_priority)?scene.source_priority.slice(0,8):[],queries_tried:Array.isArray(r.queries_tried)?r.queries_tried.slice(0,8):[],candidate_count:r.candidate_count??null,candidate_source_counts:r.candidate_source_counts||{},candidate_type_counts:r.candidate_type_counts||{},scored_count:r.scored_count??null,vision_calls:r.vision_calls??null,cache_hits:r.cache_hits??null,search_rounds:r.search_rounds??null,selected_query:r.selected_query||null,selected_source:r.source||((r.type==='template'||r.visual_source==='template')?'template':null),score:r.score??null,relevance:r.relevance??null,semantic_match:r.semantic_match??null,entity_match:r.entity_match??null,action_match:r.action_match??null,relationship_match:r.relationship_match??null,local_similarity:r.local_similarity??null,top_local_similarity:r.top_local_similarity??r.local_similarity??null,threshold:r.threshold??null,run_used:r.run_used??null,run_remaining:r.run_remaining??null,scene_used:r.scene_used??null,scene_remaining:r.scene_remaining??null,actual_video_verified:r.actual_video_verified===true,in_point_sec:r.in_point_sec??null,out_point_sec:r.out_point_sec??null,verified_frame_indices:Array.isArray(r.verified_frame_indices)?r.verified_frame_indices:[],annotation_count:Array.isArray(r.annotation_plan)?r.annotation_plan.length:0,library_hit:r.library_hit===true,degraded:false,quality_gate_passed:true,fallback_reason:null,intentional_template:false};
...
def assert_applied(workflow: dict) -> None:
    ...
    for marker in ["candidate_source_counts", "candidate_type_counts", "top_local_similarity", "requested_queries", "queries_tried", "semantic_match", "entity_match", "action_match", "relationship_match", "actual_video_verified", "verified_frame_indices", "annotation_plan", "annotated_real"]:
