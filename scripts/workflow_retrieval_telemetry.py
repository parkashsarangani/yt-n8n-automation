...
TAG_BROLL = r"""// RETRIEVAL_TELEMETRY_V1: observational trace for real-media/template-fallback selection.
...
TAG_TEMPLATE = r"""// RETRIEVAL_TELEMETRY_V1: record intentional template routing as a zero-search scene.
const scene=$('Split Out Scenes').item.json;
return {json:{scene_index:scene.scene_index,visual_source:'template',template_name:scene.template_name,template_data:scene.template_data,retrieval:{scene_index:scene.scene_index,visual_mode:scene.visual_mode||'template_explainer',must_show:scene.must_show||scene.point||null,requested_queries:[],source_priority:[],queries_tried:[],candidate_count:0,candidate_source_counts:{},candidate_type_counts:{},scored_count:0,vision_calls:0,cache_hits:0,search_rounds:0,selected_query:null,selected_source:'template',score:null,relevance:null,local_similarity:null,top_local_similarity:null,threshold:null,degraded:false,quality_gate_passed:true,fallback_reason:null,intentional_template:true}}};"""
...
def apply(workflow: dict) -> dict:
    ...
    if "retrieval: v.retrieval" not in code:
        ...
        replacement = "    template_data: v.template_data,\n    retrieval: v.retrieval || null,\n    asset_score: v.asset_score,"
    ...
    if "const retrieval_telemetry" not in code:
        ...
        replacement = "const asset_quality_avg = assetScores.length ? Number((assetScores.reduce((s, x) => s + x, 0) / assetScores.length).toFixed(2)) : null;\nconst retrieval_telemetry = merged.filter((v) => !v?.template_data?.is_outro).map((v) => v.retrieval || {scene_index:v.scene_index,selected_source:v.asset_source||v.visual_source||null});\n\nreturn {"
        if anchor not in code:
            raise RuntimeError("merge aggregate telemetry anchor missing")
        code = code.replace(anchor, replacement, 1)

        anchor = "    asset_quality_avg,\n    data: merged"
        replacement = "    asset_quality_avg,\n    retrieval_telemetry,\n    data: merged"
        if anchor not in code:
    ...
    if "retrieval_telemetry:" not in body:
        ...
        replacement = "asset_quality_avg: ($('Merge By scene_index (not position)').item.json.asset_quality_avg ?? null) }, retrieval_telemetry: ($('Merge By scene_index (not position)').item.json.retrieval_telemetry || []) }) }}"
...
def assert_applied(workflow: dict) -> None:
    ...
    for text, label in [(tag,"Tag B-roll"),(template,"Tag Template Video")]:
        if MARKER not in text:
            raise RuntimeError(f"{label} missing retrieval telemetry marker")
